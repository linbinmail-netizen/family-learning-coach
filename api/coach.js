...
function localMasterySignal(reply = "", expectedMethod = "", skill = "") {
  ...
  const guessingWithoutRejection = /猜|guess/.test(text) && !/不能.*猜|不要.*猜|不是.*猜|没有.*猜|not guess|avoid.*guess/.test(text);
  if (guessingWithoutRejection || /随便|不知道|不会|不确定|答案是|选[a-d]|choose|idk/.test(text)) return false;
  ...
  const hasMisconceptionCheck = /误区|错因|不能|不要|避免|避开|没有直接猜|不是.*猜|not guess|avoid|trap|mistake/.test(text);
  ...
  return hasMethodLanguage && hasMisconceptionCheck && (keywordHits >= 1 || text.length >= 32);
}
...
export function buildFallbackMasteryEvaluation(body = {}) {
  ...
  return {
    passed: false,
    reply: "还差避坑说明。请写第一步、原因，以及没有掉进哪个常见误区。",
    nextPrompt: "请用“先看...因为...不能只...”写一句完整方法。",
  };
}
...
export function buildMasteryEvaluationRequest(body = {}) {
  ...
  return {
    ...
    instructions: [
      ...
      "passed must also require one misconception check: the student says what trap, wrong shortcut, or guessing pattern they avoided.",
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              ...
              rubric: [
                ...
                "Does the student explain what misconception, trap, or guessing shortcut they avoided?",
